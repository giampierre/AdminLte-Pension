...
<html>
        ...
        <script>
            $(document).ready(function () {
                $("#save_data").click(function () {
                    //alert("succeess");
                    $.ajax({
                        url: "mantenimiento/cena/EditCena.jsp",
                        type: "post",
                        data: {
                            nombres: $('#nombres').val(),
                            estado: $('#estado').val(),
    ...
    <body>
        ...
        <%            
            String idCena = request.getParameter("f_id");
            String id = "";
            ...
            COMANDO = "SELECT d.idcena, p.idpensionista, nom_pen(d.idpensionista) AS Nombres, p.tipo, IF(p.tipo=0, 'General', 'Ejecutivo') as nom_tipo, "
                    + "d.estado, IF(d.estado=0, 'Cenó', 'Para llevar') AS nom_estado, "
                    + "fecha_c, cantidad, d.monto "
                    + "FROM cena d "
                    + "INNER JOIN  pensionista p "
                    + "ON (d.idPENSIONISTA = p.idPENSIONISTA) "
                    + "WHERE d.idcena = '" + idCena + "' ";
            ...
            <div class="panel-body">
                ...
                <div class="form-row" align="right">
                    <div class="form-group col-md-12">
                        ...
                        <input type="hidden" id="idcena" name="idcena" value="<%=idCena%>">
                    </div>
                </div>           
            </div>
        </form>
